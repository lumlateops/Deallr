...
class Application_Model_Account
{
	...
	function __construct($email, $email_password, $provider)
	{
		$auth_session = Zend_Registry::get('auth_session');
		$this->_session = $auth_session;
		$this->_email = $email;
		$this->_email_password = $email_password;
	...
	{		
		if( isset( $this->_session ) && isset( $this->_session->auth_user['id'] ) ) 
		{
			...
			if( isset( $api_response ) && isset( $api_response['status'][0] ) && $api_response['status'][0] == 'ok' )
			{
				 return true;
			}
		}
